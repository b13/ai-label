...
its TypoScript is included in your project - the same way you'd include any
other b13 extension's TypoScript:
...
TypoScript (confirmed by integration-testing against a real b13 project;
every other b13 extension's TypoScript, e.g. `b13/b13-baseconfig`, is
included in consuming projects the same explicit way, despite also being
...
There is deliberately no automatic, project-independent registration (e.g.
via `ExtensionManagementUtility::addTypoScriptSetup()` in `ext_localconf.php`)
- an earlier version of this feature used that, but it did not reliably
merge into a project's actual TypoScript either. Following the same explicit
import pattern every other b13 extension already uses is what's proven to
work.
